...
class MakeController extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $path = make_dirs(app_path('Http/Controllers'), $names);
        $namespace = $this->namespace($names);

        if(file_exists("{$path}/{$name}.php")) {
            $output->writeln("<error>Controller {$namespace}\\{$name} has already exist</error>");
            return Command::FAILURE;
        }

        file_put_contents("{$path}/{$name}.php", load_stub('controller', $namespace, $name));
